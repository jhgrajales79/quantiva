...
export default function MacroPage() {
  ...
  return (
    <div className="space-y-4">
      <h1 className="text-xl font-semibold text-neutral-50">Macro Dashboard</h1>
          ...
          {indicators.map((ind) => {
            const change =
              ind.value !== null && ind.previousValue !== null
                ...
                : null;
            return (
              <div key={ind.code} className="rounded-lg border border-neutral-800 bg-neutral-900 p-4">
                <div className="flex items-center justify-between">
                  <h3 className="text-sm font-medium text-neutral-300">{ind.label}</h3>
                  <span className="text-xs text-neutral-500">{ind.unit}</span>
                </div>
                {ind.unavailable ? (
                  <p className="mt-2 text-sm text-neutral-500">Dato no disponible</p>
                ) : (
                  <>
