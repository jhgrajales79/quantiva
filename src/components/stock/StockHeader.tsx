  ...
  return (
    <div className="flex flex-wrap items-start justify-between gap-4">
      <div>
        <div className="flex items-center gap-2">
          <h1 className="text-2xl font-semibold text-neutral-50">
            {symbol}
            {quote?.companyName && (
              <span className="ml-2 text-lg font-normal text-neutral-400">— {quote.companyName}</span>
            )}
          </h1>
          {watchlistLoaded && (
            <button
              ...
              className={isFavorite ? "text-amber-400" : "text-neutral-600 hover:text-amber-400"}
            >
              ★
            </button>
          )}
      ...
      <div className="flex flex-wrap items-start gap-6 text-sm">
        ...
        <div className="w-40">
          <p className="text-xs text-neutral-500">Rango de 52 semanas</p>
          {rangePct !== null ? (
            <>
              <div className="relative mt-1 h-1.5 rounded-full bg-neutral-800">
                <div
                  className="absolute top-1/2 h-3 w-3 -translate-x-1/2 -translate-y-1/2 rounded-full border-2 border-neutral-950 bg-neutral-100"
            ...
            <p className="mt-1 text-xs text-neutral-500">Dato no disponible</p>
          )}
        </div>
      </div>
    </div>
